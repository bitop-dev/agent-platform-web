...
export const runs = {
  create: (agentId: string, mission: string, baseUrl?: string) =>
    request<Run>("/api/v1/runs", {
      method: "POST",
      body: JSON.stringify({ agent_id: agentId, mission, base_url: baseUrl }),
    }),
  get: (id: string) => request<Run>(`/api/v1/runs/${id}`),
  list: () => request<{ runs: Run[] }>("/api/v1/runs"),
  events: (id: string) =>
    request<{ events: RunEvent[] }>(`/api/v1/runs/${id}/events`),
  cancel: (id: string) =>
...
export const models = {
  list: (provider?: string) =>
    request<{ models: Model[] }>(
      `/api/v1/models${provider ? `?provider=${provider}` : ""}`
    ),
};
