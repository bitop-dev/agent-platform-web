  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link to="/agents/new"><Button><Plus className="mr-2 h-4 w-4" /> New Agent</Button></Link>
      </div>
      {isLoading ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{[1,2,3].map(i => <div key={i} className="h-40 bg-muted rounded animate-pulse" />)}</div>
      ) : !data?.agents?.length ? (
          ...
          {data.agents.map((agent) => (
            <Card key={agent.id} className="group relative">
              ...
              <Button variant="ghost" size="icon"
                ...
                onClick={(e) => { e.preventDefault(); if (confirm("Delete this agent?")) deleteMut.mutate(agent.id); }}>
